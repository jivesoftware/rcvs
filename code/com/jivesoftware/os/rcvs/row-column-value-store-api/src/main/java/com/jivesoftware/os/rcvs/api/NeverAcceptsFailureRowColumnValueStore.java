...
public class NeverAcceptsFailureRowColumnValueStore<T, R, C, V> implements RowColumnValueStore<T, R, C, V, RuntimeException> {

    private final RowColumnValueStore<T, R, C, V, ?> store;
    private final ThunderingHerd thunderingHerd;

    public NeverAcceptsFailureRowColumnValueStore(RowColumnValueStore<T, R, C, V, ?> store) {
        this.store = store;
