...
public class HBaseRowColumnValueStoreInitializer implements RowColumnValueStoreInitializer<Exception> {

    static public interface HBase098RowColumnValueStoreConfig extends Config {
        ...
        @IntDefault(24)
        public int getMarshalThreadPoolSize();
    ...
    public HBaseRowColumnValueStoreInitializer(HBase098RowColumnValueStoreConfig config) {
        this(config, defaultHbaseConfig(config));
    }

    public HBaseRowColumnValueStoreInitializer(HBase098RowColumnValueStoreConfig config, org.apache.hadoop.conf.Configuration hbaseConfig) {
        this.hbaseConfig = hbaseConfig;
        this.marshalExecutor = Executors.newFixedThreadPool(config.getMarshalThreadPoolSize());
    }

    private static org.apache.hadoop.conf.Configuration defaultHbaseConfig(HBase098RowColumnValueStoreConfig config) {
    ...
    public <T, R, C, V> RowColumnValueStore<T, R, C, V, Exception> initialize(
        ...
        return new HBaseRowColumnValueStore<>(
            HConnectionManager.createConnection(hbaseConfig, Executors.newCachedThreadPool()),
